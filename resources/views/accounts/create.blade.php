...
@section('content')
	...
	<div class="row col-md-12">
		...
		<form class="form-wrapper" action="{{route('accounts.create')}}" method="post">
			<div class="form-flex-wrapper in-form-group-wrapper">
				<div class="row">
					...
					<div class="form-group col-md-2">
						...
						<input type="text" class="form-control" placeholder="Saldo" id="balance" name="balance">
					</div>
					<div class="form-group col-md-2">
						<label for="currency">Tipo de moneda</label>
						<select class="form-control" id="currency" name="currency">
							@foreach($currencies as $currency)
								<option value="{{$currency->id}}">{{ucfirst($currency->name)}} - {{$currency->symbol}}</option>
							@endforeach
						</select>
					</div>
				</div>
				...
				<div class="row">
					...
					<div class="form-group col-md-2">
						<label class="account-icon-label">
							<img class="icon-selected" src="{{config('constants.default_paths.account_icon')}}">
						</label>
					</div>
				</div>
				<div class="row">
			...
			<input type="hidden" name="icon" id="icon-input" value="{{config('constants.default_paths.account_icon')}}">
			{!! csrf_field() !!}
		</form>
	</div>
